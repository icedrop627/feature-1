...
### 수정/개선 사항
- [ ] **페이지 삭제**
  - [ ] `app/cities/[id]/page.tsx` 삭제 (도시 상세 페이지)
  - [ ] `app/cities` 폴더 전체 삭제
- [ ] **컴포넌트 삭제**
  - [ ] `components/common/rating-display.tsx` 삭제 (별점 표시 컴포넌트)
  - [ ] `components/common/weather-badge.tsx` 삭제 (날씨 뱃지)
  - [ ] `components/common/air-quality-badge.tsx` 삭제 (공기질 뱃지)
  - [ ] `components/review/review-modal.tsx` 삭제 (리뷰 모달)
  - [ ] `components/auth/auth-modal.tsx` 삭제 (더 이상 사용하지 않음)
- [ ] **컴포넌트 폴더 정리**
  - [ ] `components/common` 폴더 삭제 (내부 파일 모두 삭제됨)
  - [ ] `components/review` 폴더 삭제 (내부 파일 모두 삭제됨)
  - [ ] `components/auth` 폴더 삭제 (내부 파일 모두 삭제됨)

### 작업 완료 후 검증 사항
- [ ] `app/cities` 폴더가 존재하지 않는지 확인
- [ ] `components/common`, `components/review`, `components/auth` 폴더가 존재하지 않는지 확인
- [ ] 삭제된 컴포넌트 파일이 완전히 제거되었는지 확인
- [ ] 남아있는 파일 구조 확인: `app/page.tsx`, `app/login/page.tsx`, `app/register/page.tsx`, `app/layout.tsx`만 존재
